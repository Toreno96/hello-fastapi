...
def send_email(email_to: str, subject_template="", html_template="", environment={}):
    ...
    message = emails.Message(
        ...
        mail_from=(config.EMAILS_FROM_NAME, config.config.EMAILS_FROM_EMAIL),
    )
    smtp_options = {"host": config.config.SMTP_HOST, "port": config.config.SMTP_PORT}
    if config.config.SMTP_TLS:
        smtp_options["tls"] = True
    if config.config.SMTP_USER:
        smtp_options["user"] = config.config.SMTP_USER
    if config.config.SMTP_PASSWORD:
        smtp_options["password"] = config.config.SMTP_PASSWORD
...
def send_test_email(email_to: str):
    ...
    send_email(
        ...
        environment={"project_name": config.config.PROJECT_NAME, "email": email_to},
    )
...
def send_new_account_email(email_to: str, username: str, password: str):
    ...
    with open(Path(config.EMAIL_TEMPLATES_DIR) / "new_account.html") as f:
        template_str = f.read()
    link = config.config.SERVER_HOST
    send_email(
        email_to=email_to,
        subject_template=subject,
        html_template=template_str,
...
def verify_password_reset_token(token) -> Optional[str]:
    try:
        decoded_token = jwt.decode(
            token, config.config.SECRET_KEY, algorithms=["HS256"]
        )
